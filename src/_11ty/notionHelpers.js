...
/* const getLocalImages = async (note, property = "Images", folder) => {
    ...
    for (const img of imagesNotion) {
        ...
        // if (img.file.url.includes("secure.notion-static.com") && !process.env.NODE_ENV === "devbuild") break;

        // if (!process.env.NODE_ENV === "devbuild") {
...
async function updateReddit(notion, posts, type) {
    if (!metadata.subreddit || process.env.NODE_ENV === "development") return;
...
async function updateTweet(notion, posts, type) {
    if (!TWITTER_TOKEN || process.env.NODE_ENV === "development") return;
